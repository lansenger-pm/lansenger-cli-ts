...
```bash
lansenger config set passport_url https://your-passport.example.com   # 必須設定
...
| `calendar` | 行事曆操作 | `primary`, `create-schedule`, `fetch-schedule`, `delete-schedule`, `list-schedules`, `attendees`, `add-attendees`, `delete-attendees`, `update-attendees`, `update-schedule`, `attendee-meta` |
...
External 模式允許您透過命令列參數直接傳入 `app_token` 和 `user_token`，完全繞過憑證儲存和 OAuth2 流程。適用於：
...
| `message send-text --group` (group chat) | **Y** | N | **Y** | 個人機器人現已支援群聊 |
...
> **Personal Bot** 只能收發訊息和上傳下載檔案，無法存取通訊錄、行事曆或 OAuth2。
>
