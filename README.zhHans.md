...
| `questionnaire` | 问卷系统 | `save`, `save-questions`, `publish`, `withdraw`, `finish`, `delete`, `detail`, `brief`, `accounts`, `created-list`, `answers`, `answer-detail`, `answer-data`, `upload-url`, ... |
| `boardroom` | 会议室预定 V2 | `rooms`, `room-detail`, `schedule`, `reserve-detail`, `reserve`, `edit-reserve`, `cancel`, `confirm-sign`, `my-reserves`, `gradings`, `area-offices` |
| `personal-todo` | 用户个人待办 | `save`, `update`, `list`, `upload-resource`, `download-url`, `upload-url` |
...
| `--app-token <token>` | 使用 external 模式：直接传入 app token，不使用凭证存储 |
| `--user-token <token>` | 在 external 模式下直接传入 user token（自动注入到命令中） |
...
External 模式允许你通过命令行参数直接传入 `app_token` 和 `user_token`，完全绕过凭证存储和 OAuth2 流程。适用于：

- CI/CD 流水线，token 由外部系统管理
- 与你自己的认证系统集成
- 自行刷新的短期 token
- 使用临时 token 进行测试

使用 external 模式时：
- `--app-token` 是进入 external 模式的必需参数
- `--user-token` 可选，但对于需要用户身份的 API 推荐传入
- 传入的 `user_token` 会自动注入到所有支持的命令中
- 凭证存储操作（`config` 命令）不可用
- 不会自动刷新 token — 你需要自行保持 token 有效
